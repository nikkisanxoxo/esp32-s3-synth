...
// ---- PCM5102A I2S GPIOs ------------------------------------------------------
static constexpr gpio_num_t I2S_BCK  = GPIO_NUM_4;
static constexpr gpio_num_t I2S_LRCK = GPIO_NUM_5;
static constexpr gpio_num_t I2S_DOUT = GPIO_NUM_6;

// ---- Note event queue --------------------------------------------------------
// MIDI task writes, audio task reads — no mutex needed.
struct NoteEvent {
    bool    on;
    uint8_t note;
    uint8_t velocity;
...
// ---- Xtensa cycle counter (experiment — uncomment to enable) -----------------
...
// ---- Audio task --------------------------------------------------------------
// Stack MUST be large (32KB): adsr_init() calls adsr_to_ll<2048> which puts a
// LinkedList<float,2048> (~14KB) on the stack during Synth construction.
static void audio_task(void *)
{
    // Construct synth here — safe with 32KB stack.
    ...
    static int16_t buf[buffer_length * 2];  // BL*2 stereo int16 samples

    /* CCOUNT callback timing (experiment — uncomment to enable)
    uint32_t cb_min   = UINT32_MAX;
    uint32_t cb_max   = 0;
    uint64_t cb_total = 0;
    uint32_t cb_count = 0;
    const uint32_t report_every = (uint32_t)(3.0 * sample_rate / buffer_length);
    */
    ...
    for (;;) {
        ...
        /* CCOUNT timing (experiment — uncomment to enable)
        uint32_t t0 = get_ccount();
        */
        (*synth)(buf);
        /* CCOUNT timing (experiment — uncomment to enable)
        uint32_t elapsed = get_ccount() - t0;
        if (elapsed < cb_min) cb_min = elapsed;
        if (elapsed > cb_max) cb_max = elapsed;
        cb_total += elapsed;
        cb_count++;
        if (cb_count >= report_every) {
            uint32_t avg = (uint32_t)(cb_total / cb_count);
            printf("cb: min=%lu avg=%lu max=%lu cycles (budget=320000, n=%lu)\n",
                   cb_min, avg, cb_max, cb_count);
            cb_min = UINT32_MAX; cb_max = 0; cb_total = 0; cb_count = 0;
        }
        */
        ...
        while (tud_midi_available()) {
            ...
            switch (status & 0xF0u) {
                case 0x90u:
                    ...
                    // if (d2 > 0) ESP_LOGI(TAG, "NoteOn  %3d  vel=%d", d1, d2);
                    // else        ESP_LOGI(TAG, "NoteOff %3d (vel=0)", d1);
                    break;
                case 0x80u:
                    ev = {false, d1, 0};
                    xQueueSend(s_note_queue, &ev, 0);
                    // ESP_LOGI(TAG, "NoteOff %3d", d1);
                    break;
                default:
                    break;
            }
...
extern "C" void app_main(void)
{
    ...
    i2s_init();
    // audio_task → Core 1, priority 10: must never be preempted mid-block.
    // usb_midi_task → Core 0, priority 5: MIDI latency of a few ms is acceptable.
    // Stack 32768: adsr_init() puts a LinkedList<float,2048> (~14KB) on the stack.
    xTaskCreatePinnedToCore(audio_task,    "audio",    32768, NULL, 10, NULL, 1);
